<?php global $processNeedId?>
<script>
    var processNeedId = <?=$processNeedId?>;
</script>
<div class="block">
    <div class="head">
        加工需求报价
    ...
    <table class="table sheet">
        ...
        <tr class="tr-template">
            ...
            <td><a class="content source" data-field="quote_attachment">点击下载</a></td>
...
<script>
    ...
    $(document).ready(function () {
        ...
        TableController.init('process_quote_list', backDataHandle);
        if(processNeedId)TableController.setfilter({process_need:processNeedId});
    ...
    function registEvent() {
        $(document).on('click', '.button', function () {
            ...
            switch (type) {
                ...
                case 'delete':
                    if(confirm('确认要删除？')){
                        deleteRecord('process_need_quote',{process_need_quote_id:id},function(back){
                            showToast('删除成功');
                            TableController.getList();
                        });
                    }
        ...
        $(document).on('change','.status-select',function(){
            ...
            altTable('process_need_quote','status',value,'process_need_quote_id',id,function(){
                showToast('修改成功')
            })
        })
...
//            element.find('.proirity').attr('id', v.company_id);
            element.find('.quote').attr('id', 'quo' + v.process_need_quote_id);
            element.find('.delete').attr('id','del'+v.process_need_quote_id);
            element.find('.detail').attr('id', 'det' + v.process_need_quote_id);
            element.find('.status-select').attr('id','stu'+ v.process_need_quote_id);
