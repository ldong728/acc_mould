<div class="block">
    ...
    <table class="table sheet">
        ...
        <tr class="tr-template">
            <td class="content" data-field="bidding_inquiry_title"></td>
            <td><a class="content source">点击下载</a></td>
            <td class="content" data-field="m_requires"></td>
            <td class="content" data-field="t_requires"></td>
...
<script>
    ...
    $(document).ready(function () {
        ...
        TableController.init('bidding_inquiry_list', backDataHandle);
    ...
    function registEvent() {
        $(document).on('click', '.button', function () {
            ...
            switch (type) {
                case 'quote':
                    var href = getHref('tender_inquiry_manage');
                    location.href = href + '&bidding_inquiry_id=' + id;

                    break;
                case 'delete':
                    if(confirm('确认要删除？')){
                        deleteRecord('bidding_inquiry',{bidding_inquiry_id:id},function(back){
                            showToast('删除成功');
                            TableController.getList();
                        });
                    }
        ...
        $(document).on('change','.status-select',function(){
            ...
            altTable('bidding_inquiry','status',value,'bidding_inquiry_id',id,function(){
                showToast('修改成功')
            })
        });
    ...
    function backDataHandle(back) {
        ...
        $.each(back.list, function (k, v) {
            ...
            element.find('.quote').attr('id', 'quo' + v.bidding_inquiry_id);
            element.find('.delete').attr('id','del'+ v.bidding_inquiry_id);
            element.find('.detail').attr('id', 'det' + v.bidding_inquiry_id);
            element.find('.status-select').attr('id','stu'+ v.bidding_inquiry_id);
