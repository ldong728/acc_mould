...
class PushSender {
    ...
    function __construct($deviceToken='') {
        ...
        $strlen=strlen($this->deviceToken);
        if(44==$strlen){
            $this->type='android';
            $this->appkey=APP_KEY_ANDROID;
            $this->appMasterSecret=MASTER_SECRET_ANDROID;
        }else if(64==$strlen){
            $this->type='ios';
            $this->appkey=APP_KEY_IOS;
            $this->appMasterSecret=MASTER_SECRET_IOS;
        }else{
    ...
    public function sendUniCast($ticker,$title,$text){
        if('android'==$this->type){
            return $this->sendAndroidUnicast($this->deviceToken,$ticker,$title,$text);
        }else{
            return $this->sendIosUnicast($this->deviceToken,$ticker,$title,$text);
        }
    }

    private function sendAndroidBroadcast() {
        try {
            ...
            mylog("Sending broadcast notification, please wait...\r\n");
            $brocast->send();
            mylog("Sent SUCCESS\r\n");
        } catch (Exception $e) {
    ...
    private function sendAndroidUnicast($deviceToken,$ticker,$title,$text) {
        try {
            ...
            // Set extra fields
            $unicast->setExtraField("test", "none");
//            mylog("Sending unicast notification, please wait...\r\n");
            $result=$unicast->send();
            return $result;
//            mylog("Sent SUCCESS\r\n");
        } catch (Exception $e) {
    ...
    private function sendAndroidFilecast() {
        try {
            ...
            mylog("Uploading file contents, please wait...\r\n");
            // Upload your device tokens, and use '\n' to split them if there are multiple tokens
            $filecast->uploadContents("aa"."\n"."bb");
//            mylog("Sending filecast notification, please wait...\r\n");
            $filecast->send();
//            mylog("Sent SUCCESS\r\n");
        } catch (Exception $e) {
            mylog("Caught exception: " . $e->getMessage());
    ...
    private function sendAndroidGroupcast() {
        try {
            /*
            ...
            mylog("Sending groupcast notification, please wait...\r\n");
            $groupcast->send();
            mylog("Sent SUCCESS\r\n");
        } catch (Exception $e) {
            mylog("Caught exception: " . $e->getMessage());
    ...
    private function sendAndroidCustomizedcast() {
        try {
            ...
            mylog("Sending customizedcast notification, please wait...\r\n");
            $customizedcast->send();
            mylog("Sent SUCCESS\r\n");
        } catch (Exception $e) {
            mylog("Caught exception: " . $e->getMessage());
    ...
    private function sendAndroidCustomizedcastFileId() {
        try {
            $customizedcast = new AndroidCustomizedcast();
            $customizedcast->setAppMasterSecret($this->appMasterSecret);
            $customizedcast->setPredefinedKeyValue("appkey",           $this->appkey);
    ...
    private function sendIOSBroadcast() {
        try {
    ...
    private function sendIOSUnicast($deviceToken,$ticker,$title,$text) {
        try {
...
//            return ['']
    ...
    private function sendIOSFilecast() {
        try {
    ...
    private function sendIOSGroupcast() {
        try {
            /*
              *  Construct the filter condition:
              *  "where":
    ...
    private function sendIOSCustomizedcast() {
        try {
